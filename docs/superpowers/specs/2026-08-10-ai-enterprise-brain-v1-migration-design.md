...
### 2.2 扁平化管理与传统考勤退役（2026-08-10 修订）

- 量子星河采用以结果为核心的扁平化管理，不限制员工的工作时间和地点，不建设上下班打卡、排班、迟到早退、工时限制、定位/Wi-Fi 打卡、补卡或考勤加班流程。
- 每个人对分配任务的截止时间、成果提交和验收结果负责；所有岗位都必须能进入任务管理。
- 第一阶段只下线可见的“考勤管理”入口，把员工菜单“我的考勤”替换为“我的任务”，并将旧 `/attendance` 地址重定向到 `/tasks`。本阶段不新增任务后端，也不迁移或删除旧考勤代码和本地数据。
- 旧考勤代码暂时保留，是因为历史演示薪资、通知等模块仍可能引用；后续业务迁移时统一移除失效考勤代码、数据以及与薪资的耦合。
- 请假是独立的人事协同事项，不属于考勤跟踪，不因传统考勤退役而取消。
...
- Dashboard、项目、任务、活动、组织人事、请假、薪资、审批、客户、分析、知识和设置页面；旧考勤页面仅保留兼容重定向和待清理代码，不再作为产品能力复用。
...
| /attendance | 仅兼容旧收藏地址，服务端重定向到 `/tasks`，不再提供考勤能力 |
...
- 接入 Dify 知识库。
- 建立 outbox、n8n 和飞书通知。
- 完成人事、请假、薪资、审批、活动、客户、分析、设置和帮助的数据迁移与内容清理；同时移除旧考勤代码、数据及其与薪资/通知的历史耦合。
