...
- Every database `organization.manage` authorization path (claim, exact apply, fenced apply/replay, resolution and RLS) joins the authoritative role row and requires an enabled role whose tenant is exact and whose organization is global or the exact target organization. Disabled roles and roles owned by another organization never authorize reads, actor selection or mutation. Incremental claim resolves and locks the connection directly from the durable cursor; unscoped full/reconcile chooses an eligible connection instead of letting an older active/backoff connection starve another ready connection, and emits a no-work reason only after proving no connection is eligible. Claim, apply and finish use one documented `connection -> lease -> run` row-lock order so overlapping cron and manager operations cannot deadlock; behavioral two-session coverage must prove this ordering.
