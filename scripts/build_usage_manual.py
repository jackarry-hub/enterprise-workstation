...
def add_role_table(doc):
    headers = ["试用身份", "岗位角色", "首次进入重点", "主要职责"]
    rows = [
        ["李总", "决策人", "AI 决策调度台", "下达目标、确认拆解、处理风险、总验收"],
        ["张伟", "部门负责人", "负责人推进台", "承接目标、调度资源、验收成果、团队审批"],
        ["陈晨", "员工", "我的执行台", "执行任务、上报阻塞、上传成果、提交验收"],
        ["周倩", "财务", "财务工作台", "预算协同、薪资核算与发放"],
        ["李琪", "人事", "人事工作台", "人员支持、考勤封账、薪资复核"],
    ]
...
def add_cover(doc):
    ...
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r = p.add_run("从领导命令到个人执行、验收、审批与复盘的完整闭环")
    ...
    values = [
        ("系统地址", "http://localhost:3007"),
        ("数据方式", "本地浏览器持久化（业务记录 + 上传文件）"),
        ("交付日期", "2026 年 8 月 9 日"),
    ]
...
def build_document():
    ...
    heading = doc.add_heading("2. 启动、登录与角色切换", level=1)
    heading.paragraph_format.page_break_before = True
    add_para(doc, "在项目目录依次执行以下命令：")
    add_callout(doc, "启动命令", "npm install\nnpm run dev -- -p 3007")
    add_para(doc, "浏览器打开 http://localhost:3007。点击右上角用户菜单，可切换五类试用身份。")
    add_callout(
        doc,
        "正式上线提醒",
        "当前身份切换用于本地流程验收。面向真实企业部署时，应接入统一身份认证或 Supabase Auth，并将业务数据与文件迁移到服务端。",
    )
    ...
    doc.add_heading("5.2 考勤与个人事项", level=2)
    add_bullets(doc, [
        "考勤管理：签到、签退、补卡和加班申请。",
    ...
    add_callout(doc, "薪资顺序", "考勤封账 → 财务核算 → 人事复核 → 领导批准 → 财务发放。系统不允许跳步。")
    ...
    add_para(doc, "客户管理支持新建客户、筛选、查看档案、记录跟进并推进客户阶段。客户资料保存在当前浏览器，刷新后仍然存在。")
    ...
    add_para(doc, "本地交付版把业务记录保存在 localStorage，上传文件保存在 IndexedDB。普通刷新不会清除；清理浏览器站点数据、更换浏览器或更换电脑会导致本地数据不可用。")
    ...
    questions = [
        ...
        ("为什么不能进行薪资核算？", "人事必须先处理所有考勤异常并完成考勤封账。"),
        ("为什么页面自动跳回首页？", "当前角色没有该页面权限，请从当前岗位导航进入允许模块。"),
        ("示例文件为什么不能下载？", "内置示例记录只有文件元数据；用户自己上传的文件包含真实内容，可以下载。"),
    ]
