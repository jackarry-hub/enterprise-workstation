...
select member.tenant_id,member.organization_id,member.id,'ORG-'||member.id,'Organization member','Tester','active','{}'::text[] from public.organization_members member where member.user_id between '97000000-0000-4000-8000-000000000001'::uuid and '97000000-0000-4000-8000-000000000003'::uuid;
...
select ok(exists (select 1 from pg_constraint where conrelid='public.organization_command_idempotency'::regclass and contype='f'),'idempotency ledger has tenant organization ownership foreign key');
