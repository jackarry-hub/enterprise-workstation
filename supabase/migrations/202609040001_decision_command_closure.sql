...
begin
  ...
  insert into public.decision_archives(tenant_id,organization_id,command_id,project_id,knowledge_document_id,summary,metrics,archived_by_member_id)
  values(v_actor.tenant_id,v_actor.organization_id,v_command.id,v_project.id,v_document.id,btrim(p_summary),jsonb_build_object('taskTotal',v_total,'taskDone',v_done),v_actor.member_id) returning * into v_archive;
...
begin
  ...
  begin
    ...
    for v_item in select value from jsonb_array_elements(v_plan.plan->'milestones') loop
      ...
      values(v_actor.tenant_id,v_actor.organization_id,v_project.id,v_owner,btrim(v_item->>'name'),coalesce(v_item->>'description',''),'pending',current_date,(v_item->>'dueDate')::date,0,jsonb_object_length(v_milestone_ids),v_actor.member_id,v_actor.member_id,1) returning * into v_milestone;
