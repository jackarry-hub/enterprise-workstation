...
export function EmployeeBasicInfo({ employee }: { employee: EmployeeDirectoryItem }) {
  ...
  const rows = [
    { label: "企业邮箱", value: profile.workEmail ?? "未绑定", icon: Mail },
    { label: "联系电话", value: profile.phone ?? "未记录", icon: Phone },
    { label: "员工工号", value: profile.employeeNo, icon: IdCard },
    { label: "入职日期", value: formatDate(profile.hireDate), icon: CalendarDays },
  ];

  return (
    <GlassCard className="p-5 sm:p-6">
      <h2 className="text-lg font-semibold text-foreground">基本信息</h2>
      <p className="mt-1 text-xs text-muted-foreground">员工联系信息与基础档案</p>
