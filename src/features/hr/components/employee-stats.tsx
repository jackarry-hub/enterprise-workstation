...
export function EmployeeStats({ stats }: { stats: EmployeeDirectoryStats }) {
  ...
  return (
    <section aria-label="员工统计" className="grid grid-cols-2 gap-3 xl:grid-cols-4">
      <DataCard compact icon={UsersRound} label="员工总数" value={String(stats.total)} trend="+2" trendLabel="本月净增" tone="blue" />
      ...
      <DataCard compact icon={Building2} label="部门数量" value={String(stats.departments)} trend="全覆盖" trendLabel="组织架构" tone="purple" />
    </section>
  );
}
