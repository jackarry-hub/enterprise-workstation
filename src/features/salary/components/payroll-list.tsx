...
function salaryGradeText(record: SalaryRecord) {
  if (!record.employee.salaryGradeCode && !record.employee.jobLevel) return record.employee.employeeNo;
  return [
    record.employee.salaryGradeCode,
    record.employee.jobLevel ? `L${record.employee.jobLevel}` : null,
  ].filter(Boolean).join(" · ");
}
