...
function statsFromRecords(records: SalaryResult["data"]["records"]) {
...
export function PayrollWorkspace({ result }: { result: SalaryResult }) {
  ...
  const canManageSalary = session.permissionCodes.includes("salary.manage")
    || ["executive", "finance", "hr"].includes(actor.role);
  const cycleStatus: SalaryStatus = state.payrollRun.status === "draft" ? "draft" : state.payrollRun.status === "paid" ? "paid" : "processing";
  const cycleRecords = useMemo(() => isSupabaseData ? result.data.records : result.data.records.map((record) => record.month === state.payrollRun.month ? { ...record, status: cycleStatus, paidAt: cycleStatus === "paid" ? state.payrollRun.paidAt ? new Date(state.payrollRun.paidAt).toLocaleString("zh-CN") : record.paidAt : undefined, history: record.history.map((item) => item.month === state.payrollRun.month ? { ...item, status: cycleStatus } : item) } : record), [cycleStatus, isSupabaseData, result.data.records, state.payrollRun.month, state.payrollRun.paidAt]);
  const visibleRecords = useMemo(() => {
    if (isSupabaseData) {
      return canManageSalary
        ? cycleRecords
        : cycleRecords.filter(({ employee }) => employee.id === session.member.employeeProfileId);
    }
    if (!isFixtureBound) return [];
    return actor.role === "employee"
      ? cycleRecords.filter(({ employee }) => employee.displayName === actor.name)
      : cycleRecords;
  }, [actor.name, actor.role, canManageSalary, cycleRecords, isFixtureBound, isSupabaseData, session.member.employeeProfileId]);
  const records = useMemo(() => filterSalaryRecords(visibleRecords, filters), [filters, visibleRecords]);
  const visibleStats = useMemo(() => {
    if (isSupabaseData) return statsFromRecords(visibleRecords);
    if (!isFixtureBound) return { totalSalary: 0, employeeCount: 0, averageSalary: 0 };
    return actor.role === "employee"
      ? statsFromRecords(visibleRecords)
      : {
        totalSalary: state.payrollRun.grossAmount,
        employeeCount: state.payrollRun.headcount,
        averageSalary: state.payrollRun.headcount ? Math.round(state.payrollRun.grossAmount / state.payrollRun.headcount) : 0,
      };
  }, [actor.role, isFixtureBound, isSupabaseData, state.payrollRun.grossAmount, state.payrollRun.headcount, visibleRecords]);
  return (
    <main className="mx-auto flex w-full max-w-420 flex-col gap-4 px-3 pt-5 pb-26 sm:px-4 lg:px-5 lg:pt-9 lg:pb-6">
      ...
      {!isSupabaseData && !isFixtureBound ? <RealDataNotice message="当前账号没有可显示的真实薪资数据。" /> : isSupabaseData ? <GlassCard className="p-4 text-sm text-muted-foreground"><strong className="text-foreground">真实薪资模式：</strong>{canManageSalary ? "按权限展示可管理员工工资；基础工资来自部门 × 职级政策，奖金来自项目奖金池与绩效项。" : `仅展示 ${actor.name} 本人的工资记录，其他员工数据已按权限隐藏。`}</GlassCard> : actor.role !== "employee" ? <PayrollControlPanel /> : <GlassCard className="p-4 text-sm text-muted-foreground"><strong className="text-foreground">个人工资单模式：</strong>仅展示 {actor.name} 本人的工资记录，其他员工数据已按权限隐藏。</GlassCard>}
      ...
      <section className="grid min-w-0 gap-4 xl:grid-cols-12">
        <GlassCard className="min-w-0 overflow-hidden p-3 sm:p-4 xl:col-span-9">
          <div className="flex flex-col gap-1 px-1 pb-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-lg font-semibold text-foreground">工资发放记录</h2><p className="mt-0.5 text-xs text-muted-foreground">按员工、月份和状态核对工资单</p></div><span className="text-xs text-muted-foreground">共 {visibleStats.employeeCount} 名员工</span></div>
          {(isSupabaseData ? canManageSalary : isFixtureBound && actor.role !== "employee") ? <PayrollFilters departments={result.data.departments} filters={filters} onChange={setFilters} onReset={() => setFilters(defaultFilters)} /> : null}
