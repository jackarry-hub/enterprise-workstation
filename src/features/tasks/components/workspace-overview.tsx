...
export function WorkspaceOverview({ overview }: { overview: WorkspaceData["overview"] }) {
  const items = [
    { label: "今日任务", value: overview.todayTaskCount, suffix: "项", icon: ListChecks, tone: "text-primary bg-primary/10" },
    { label: "待审批", value: overview.pendingApprovalCount, suffix: "项", icon: ClipboardCheck, tone: "text-success bg-success/10" },
    { label: "截止提醒", value: overview.deadlineReminderCount, suffix: "项", icon: CalendarClock, tone: "text-warning bg-warning/10" },
    { label: "本周完成率", value: overview.weeklyCompletionRate, suffix: "%", icon: TrendingUp, tone: "text-chart-3 bg-chart-3/10" },
  ];
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 xl:grid-cols-4">
        {items.map(({ label, value, suffix, icon: Icon, tone }) => (
          <div key={label} className="flex items-center gap-3 rounded-2xl border border-glass-border bg-background/62 p-3 sm:p-4">
            ...
            <div><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 text-2xl font-semibold tracking-tight text-foreground">{value}<span className="ml-1 text-xs font-normal text-muted-foreground">{suffix}</span></p></div>
          </div>
        ))}
      </div>
    </GlassCard>
