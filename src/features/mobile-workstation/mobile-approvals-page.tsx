...
import { useDemoApprovals } from "@/features/approvals/approval-demo-state";
...
export function MobileApprovalsPage({ result }: { result: ApprovalResult }) {
  ...
  const approvals = useDemoApprovals(result.data.approvals, result.source === "mock");
  const pending = approvals.filter((approval) => approval.status === "pending" && approval.owner.displayName === actor.name);
  const mine = approvals.filter((approval) => approval.applicant.displayName === actor.name || approval.initiatedByViewer);
  const visible = tab === "mine" ? mine : pending;
  return (
    <main className="mobile-page">
      <header className="mobile-page-header"><div><h1>审批</h1><p>及时处理需要你确认的事项</p></div></header>
      <div role="tablist" aria-label="审批分类" className="mobile-segmented-tabs"><button role="tab" aria-label="待我审批" aria-selected={tab === "pending"} onClick={() => setTab("pending")}>待我审批 <span aria-hidden="true">{pending.length}</span></button><button role="tab" aria-label="我发起的" aria-selected={tab === "mine"} onClick={() => setTab("mine")}>我发起的 <span aria-hidden="true">{mine.length}</span></button></div>
