  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-2xl">
        ...
        <div className="rounded-2xl border border-border/70 bg-white/55 p-4"><h3 className="font-semibold">关联项目</h3><div className="mt-3 space-y-2">{relatedProjects.length ? relatedProjects.map(({ project }) => <Button key={project.id} asChild variant="outline" className="w-full justify-between"><Link href={`/projects/${project.id}`}>{project.name}<span>{project.progress}%</span></Link></Button>) : <p className="text-sm text-muted-foreground">暂无关联项目</p>}</div></div>
        <div className="rounded-2xl border border-border/70 bg-white/55 p-4"><div className="flex items-center justify-between gap-2"><h3 className="font-semibold">跟进记录</h3><Button type="button" size="sm" disabled={customer.status === "won"} onClick={() => onAdvance(customer.id)}>{customer.status === "won" ? "已成交" : "推进阶段"}</Button></div><div className="mt-3 flex gap-2"><Textarea aria-label="新增客户跟进记录" value={note} onChange={(event) => setNote(event.target.value)} placeholder="记录本次沟通结果和下一步动作" className="min-h-10 flex-1" /><Button type="button" variant="outline" onClick={() => { if (!note.trim()) { setError("请先填写跟进内容"); return; } onAddFollowUp(customer.id, note.trim()); setNote(""); setError(""); }}>保存跟进</Button></div>{error ? <p role="alert" className="mt-2 text-xs text-destructive">{error}</p> : null}<div className="mt-4 space-y-3">{customer.activities.map((activity) => <div key={activity.id}><p className="text-sm">{activity.content}</p><p className="mt-0.5 text-xs text-muted-foreground">{activity.createdAt}</p></div>)}</div></div>
      </DialogContent>
    </Dialog>
  );
}
