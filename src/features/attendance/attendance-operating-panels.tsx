...
export function TodayClockPanel() {
  ...
  return <GlassCard className="overflow-hidden border-primary/20">
    <div className="flex flex-col gap-3 border-b border-border/70 bg-brand-soft/50 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
      ...
      <select aria-label="选择打卡方式" value={method} onChange={(event) => setMethod(event.target.value as typeof method)} className="h-9 rounded-xl border border-input bg-background px-3 text-xs"><option value="office_wifi">办公 Wi-Fi</option><option value="mobile_gps">手机定位</option><option value="web">量子智枢网页</option></select>
    </div>
...
export function AttendancePolicyAndPeriod() {
  ...
  return <section id="monthly-close" className="grid scroll-mt-24 gap-3 xl:grid-cols-[1.35fr_1fr]">
