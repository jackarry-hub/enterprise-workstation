...
export function SettingsWorkspace() {
  ...
  useEffect(() => {
    ...
    if (requestedTab === "enterprise" || requestedTab === "personal" || requestedTab === "notifications" || requestedTab === "permissions") {
    ...
    <main className="mx-auto flex w-full max-w-420 flex-col gap-4 px-3 pt-5 pb-26 sm:px-4 lg:px-5 lg:pt-9 lg:pb-6">
      <section className="relative overflow-hidden rounded-3xl border border-glass-border bg-background px-5 py-6 shadow-[0_18px_50px_rgba(60,105,170,0.08)] sm:px-7">
        ...
        <div className="relative max-w-4xl">
          <PageHeader
            title="系统设置"
            description="管理企业信息、个人资料、通知与系统基础偏好。"
            actions={<Badge variant="info" className="h-8 gap-1.5 rounded-xl px-3"><Settings2 aria-hidden="true" className="size-3.5" />基础配置</Badge>}
          />
        </div>
      </section>

      <GlassCard className="min-w-0 overflow-hidden p-3 sm:p-4">
        <Tabs value={activeTab} onValueChange={changeTab} className="gap-4 xl:grid xl:grid-cols-[220px_minmax(0,1fr)] xl:items-start">
          <TabsList className="w-full justify-start overflow-x-auto rounded-2xl bg-muted/55 p-1 xl:sticky xl:top-22 xl:flex xl:flex-col xl:items-stretch xl:gap-1 xl:overflow-visible xl:p-2">
            {tabItems.map(({ value, label, icon: Icon }) => (
              <TabsTrigger key={value} value={value} className="h-10 gap-2 rounded-xl px-3 xl:flex-none xl:justify-start">
          ...
          <div className="min-w-0 rounded-3xl border border-glass-border bg-background/50 p-4 sm:p-5 xl:min-h-140">
      ...
      <GlassCard className="grid gap-3 p-4 sm:grid-cols-3">
        {[
          ...
          { icon: Building2, label: "企业时区", value: settings.organization.timezone === "asia-shanghai" ? "Asia/Shanghai" : "Asia/Singapore" },
